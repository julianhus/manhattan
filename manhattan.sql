...
CREATE INDEX `fk_producto_tienda1_idx` ON `mydb`.`producto` (`id_tienda` ASC);
...
CREATE INDEX `fk_usuario_has_producto_producto1_idx` ON `mydb`.`mercado` (`id_producto` ASC);
...
CREATE UNIQUE INDEX `id_producto_UNIQUE` ON `mydb`.`mercado` (`id_producto` ASC);
